...
def actualizar_bases_reclamos(todos_df, spreadsheet):
    ...
    for _, row in cerrados_df.iterrows():
        fila = {
            ...
            "VALOR RECLAMO": pd.to_numeric(row.get("VALOR SINIESTRO", 0), errors="coerce") or 0.0,
            "DEDUCIBLE": pd.to_numeric(row.get("DEDUCIBLE", 0), errors="coerce") or 0.0,
            "LIQUIDADO": pd.to_numeric(row.get("LIQUIDACION", 0), errors="coerce") or 0.0,
    ...
    for _, row in pendientes_df.iterrows():
        fila = {
            ...
            "VALOR SINIESTRO": pd.to_numeric(row.get("VALOR SINIESTRO", 0), errors="coerce") or 0.0,
            "DEDUCIBLE": pd.to_numeric(row.get("DEDUCIBLE", 0), errors="coerce") or 0.0,
            "RASA": pd.to_numeric( row.get("RASA", ""),errors="coerce") or 0.0,
            "LIQUIDACION": pd.to_numeric(row.get("LIQUIDACION", 0), errors="coerce") or 0.0,
            "TALLER DE REPARACION": row.get("TALLER", ""),
            "CAUSA": row.get("CAUSA", ""),
            "MES": row.get("MES", ""),
